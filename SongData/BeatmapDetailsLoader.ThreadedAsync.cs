...
namespace EnhancedSearchAndFilters.SongData
{
    public partial class BeatmapDetailsLoader
    {
        ...
        private class ThreadedAsyncCacher : ICacher
        {
            ...
            public void StopCaching()
            {
                ...
                Logger.log.Debug("Threaded async caching operation successfully cancelled");
            }
            ...
            private async Task DoCachingAsync()
            {
                try
                {
                    ...
                    if (loadedCache != null && loadedCache.Count > 0)
                    {
                        Logger.log.Info($"Retrieve {loadedCache.Count} cached beatmap details from file");
                    ...
                    List<Task<BeatmapDetails>> taskList = new List<Task<BeatmapDetails>>(WorkChunkSize);
                        ...
                        for (int i = 0; i < WorkChunkSize && index < allCustomLevels.Count; ++index)
                        {
                            ...
                            if (status == SongDataCoreDataStatus.Success)
                            {
                                if (beatmapDetails.DifficultyBeatmapSets.Any(set => set.DifficultyBeatmaps.Any(diff => diff.NoteJumpMovementSpeed == 0)))
                                {
                                    ...
                                    taskList.Add(BeatmapDetails.CreateBeatmapDetailsFromFilesAsync(level as CustomPreviewBeatmapLevel));
                                    ++i;
                                }
                                else
                                {
        ...
        private class ThreadedAsyncLoader : MonoBehaviour, ILoader
        {
            ...
            {
                DoLoadingTask(obj).Wait();
            }

            private async Task DoLoadingTask(object obj)
            {
                try
                {
                    ...
                    List<Task<OrderedBeatmapDetails>> taskList = new List<Task<OrderedBeatmapDetails>>(WorkChunkSize);
                    int index = 0;
                    while (index < levels.Length)
                    {
                            ...
                            else if (level is CustomPreviewBeatmapLevel customLevel)
                            {
                                ...
                                if (status == SongDataCoreDataStatus.Success)
                                {
                                    ...
                                    if (beatmapDetails.DifficultyBeatmapSets.Any(set => set.DifficultyBeatmaps.Any(diff => diff.NoteJumpMovementSpeed == 0)))
                                    {
                                        ...
                                        taskList.Add(GetOrderedCustomBeatmapDetailsAsync(customLevel, index));
                                        ++i;
                                    }
                                    else
                                    {
                        ...
                        while (taskList.Count > 0)
                        {
                            ...
                            // don't need to check for null BeatmapDetails here (it will be done later)
                            OrderedBeatmapDetails orderedBeatmapDetails = await finishedTask;
                            loadedLevelsUnsorted.Add(orderedBeatmapDetails);
                            _levelsLoadedCount = loadedLevelsUnsorted.Count;
            ...
            private static async Task<OrderedBeatmapDetails> GetOrderedCustomBeatmapDetailsAsync(CustomPreviewBeatmapLevel level, int index)
            {
                var beatmapDetails = await BeatmapDetails.CreateBeatmapDetailsFromFilesAsync(level);
                BeatmapDetailsLoader._cache[beatmapDetails.LevelID] = beatmapDetails;
                return new OrderedBeatmapDetails(index, beatmapDetails);
            }
